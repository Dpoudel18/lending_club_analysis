...
if page == "Loan Segment by Grade":
    ...
    st.markdown("Majority of the loans have grade of A and B, which is good for the company.")
...
if page == "Borrower's Employer and Good Loan": 
    ...
    st.markdown('Since the column emp_title is composed from an open ended question, this is not a categorical column and not much could be deduced from it. However, it is interesting to observe that most of the borrowers belong to U.S. Military and big corporations.')
...
if page == "Employment Duration and Loan Type":  
    ...
    st.markdown("The lowest percentage of 'Good Loans' or the highest percentage of 'Bad Loans' exist in the category of missing values where people don't report their employment length. It is highly likely they don't have a job to pay their loans on time. Other than that, there doesn\'t seem to be a significant pattern.")
...
if page == "Purpose for Bad Loans":
    ...
    fig.update_layout(title = {'font_color':'White', 'font_size':25})
    st.plotly_chart(fig)
    st.markdown("It is not surprising to see that the main purpose for 'Bad Loans' is for small business purpose with 26%. Small businesses often involve a lot of risks with a poor cash flow. Renewable energy sits at the second spot. However, the count for renewable energy loans is only 19. Hence, it is still too early to decipher something out of it. Educational purposes is also not surpising to see since it is more of a long-term investment. Looks like the option 'other' is also prone to 'Bad Loans'. ")
